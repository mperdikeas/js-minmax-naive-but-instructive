...
function minmaxRecur <GameStateGTP, MoveGTP>
    ...
    : TMinMaxResult<MoveGTP> {
        ...
        function _minmaxRecur(gameState           : GameStateGTP
                              , pliesRemaining    : number
                              , maximizing        : boolean
                             ): EvaluationAndMove<MoveGTP> {
                                 if (statisticsHook!=null) statisticsHook.totalNodesVisited++;
                                 if (gameRules.isTerminalState(gameState) || (pliesRemaining===0)) {
                                     if (statisticsHook!=null) statisticsHook.leafNodesEvaluated++;
                                     ...
                                     if (maximizing) {
                                         ...
                                         for (let i = 0; i < NUM_OF_MOVES ; i++) {
                                             ...
                                             const nextStateEval: ?EvaluationAndMove<MoveGTP> = _minmaxRecur(nextState, pliesRemaining-1, !maximizing);
        ...
        const evalAndMove :EvaluationAndMove<MoveGTP> = _minmaxRecur(gameState, plies, true); // in the min-max algorithm the player who starts first is the maximizing player
...
(minmaxRecur: MinMaxFT<mixed, mixed>)

exports.minmaxRecur = minmaxRecur;
